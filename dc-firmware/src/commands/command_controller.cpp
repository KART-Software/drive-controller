...
void sendConfigResponse(CommandContainer* c, uint32_t id, bool ok) {
    dc_Config cfg = c->configurator.config;
    cfg.config_changed = c->configurator.configChanged;
...
void setAppsMin(void* ctx, const dc_Command& cmd) {
    ...
    p.apps1_min = c->configurator.config.sensor_values.apps1_min;
    p.apps2_min = c->configurator.config.sensor_values.apps2_min;
    p.ittr_min  = c->configurator.config.sensor_values.ittr_min;
...
void setAppsMax(void* ctx, const dc_Command& cmd) {
    ...
    p.apps1_max = c->configurator.config.sensor_values.apps1_max;
    p.apps2_max = c->configurator.config.sensor_values.apps2_max;
    p.ittr_max  = c->configurator.config.sensor_values.ittr_max;
...
void setTpsMin(void* ctx, const dc_Command& cmd) {
    ...
    p.tps1_min = c->configurator.config.sensor_values.tps1_min;
    p.tps2_min = c->configurator.config.sensor_values.tps2_min;
...
void setTpsMax(void* ctx, const dc_Command& cmd) {
    ...
    p.tps1_max = c->configurator.config.sensor_values.tps1_max;
    p.tps2_max = c->configurator.config.sensor_values.tps2_max;
...
void setIdling(void* ctx, const dc_Command& cmd) {
    ...
    p.idling = c->configurator.config.sensor_values.target_tp_idling;
...
void setEtcTargetBound(void* ctx, const dc_Command& cmd) {
    ...
    float idling       = d.has_idling        ? d.idling        : c->configurator.config.sensor_values.target_tp_idling;
    float normalMax    = d.has_normal_max    ? d.normal_max    : c->configurator.config.sensor_values.target_tp_normal_max;
    float restrictedMax = d.has_restricted_max ? d.restricted_max : c->configurator.config.sensor_values.target_tp_restricted_max;
    c->configurator.setTargetBound(idling, normalMax, restrictedMax);
    dc_TargetBoundResponse p = dc_TargetBoundResponse_init_zero;
    p.idling         = c->configurator.config.sensor_values.target_tp_idling;
    p.normal_max     = c->configurator.config.sensor_values.target_tp_normal_max;
    p.restricted_max = c->configurator.config.sensor_values.target_tp_restricted_max;
...
void setPlausibilityCheckFlags(void* ctx, const dc_Command& cmd) {
    ...
    dc_EtcPlausibilityCheckFlags flags = c->configurator.config.etc_config.plausibility_check_flags;
    if (d.has_apps)    flags.apps    = d.apps;
    if (d.has_tps)     flags.tps     = d.tps;
    if (d.has_apps1)   flags.apps1   = d.apps1;
    if (d.has_apps2)   flags.apps2   = d.apps2;
    if (d.has_tps1)    flags.tps1    = d.tps1;
    if (d.has_tps2)    flags.tps2    = d.tps2;
    if (d.has_target)  flags.target  = d.target;
    if (d.has_bps)     flags.bps     = d.bps;
    if (d.has_bps_tps) flags.bps_tps = d.bps_tps;
...
void setEtcPid(void* ctx, const dc_Command& cmd) {
    ...
    dc_EtcPid pid = c->configurator.config.etc_config.pid;
    if (d.has_k_p) pid.k_p = d.k_p;
    if (d.has_k_i) pid.k_i = d.k_i;
    if (d.has_k_d) pid.k_d = d.k_d;
...
void setEtcTargetCurve(void* ctx, const dc_Command& cmd) {
    ...
    dc_EtcTargetCurve curve = c->configurator.config.etc_config.target_curve;
    if (d.has_a4) curve.a4 = d.a4;
    if (d.has_a3) curve.a3 = d.a3;
    if (d.has_a2) curve.a2 = d.a2;
    if (d.has_a1) curve.a1 = d.a1;
...
void setGpsGear(void* ctx, const dc_Command& cmd) {
    ...
    p.gear = gear;
    const dc_GpsCalib& gc = c->configurator.config.gps_calib;
